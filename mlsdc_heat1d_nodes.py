...
output = call("heat1d_mlsdc", ["--dt", str(dt), "--num-nodes", "3"])
ar_3 = abs_res(output)
output = call("heat1d_mlsdc", ["--dt", str(dt), "--num-nodes", "4"])
ar_4 = abs_res(output)
output = call("heat1d_mlsdc", ["--dt", str(dt), "--num-nodes", "5"])
ar_5 = abs_res(output)
...
plt.ylabel("abs res")
plt.legend()
plt.title(r"MLSDC@Heat1D, dt={}, Tend={}, dof={}, max\_iter={}, tol={}$".format(dt, tend, ndofs, num_iters, abs_res_tol))
plt.savefig("mlsdc_heat1d_nodes.pdf")
