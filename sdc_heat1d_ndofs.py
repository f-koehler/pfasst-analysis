...
output = call("heat1d_sdc", ["--dt", "0.05", "--ndofs", "16"])
ar_16 = abs_res(output)
output = call("heat1d_sdc", ["--dt", "0.05", "--ndofs", "64"])
ar_64 = abs_res(output)
output = call("heat1d_sdc", ["--dt", "0.05", "--ndofs", "128"])
ar_128 = abs_res(output)
output = call("heat1d_sdc", ["--dt", "0.05", "--ndofs", "512"])
...
plt.ylabel("abs res")
plt.legend()
plt.title(r"SDC@Heat1D, dt={}, Tend={}, nodes={}, max\_iter={}, tol={}$".format(dt, tend, num_nodes, num_iters, abs_res_tol))
plt.savefig("sdc_heat1d_ndofs.pdf")
